...
Bitcoin.prototype._initCaches = function() {
  ...
  this.utxosCache = LRU(50);
  this.txidsCache = LRU(50);
  this.balanceCache = LRU(50);
  this.summaryCache = LRU(50);
  this.blockOverviewCache = LRU(50);
  this.transactionDetailedCache = LRU(50);

  // caches valid indefinitely
  this.transactionCache = LRU(50);
  this.rawTransactionCache = LRU(50);
  this.blockCache = LRU(50);
...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  var configPath = path.resolve(spawnOptions.datadir, './mangacoin.conf');

  log.info('Using mangacoin.config file:', configPath);
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of mangacore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of mangacore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of mangacore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to bitcoind from mangacore. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in mangacoin.conf'
  );
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that bitcoind is undergoing a reindex. ' +
             ...
             'of mangacore-node services will start.');
    node._reindex = true;
  }
};
...
Bitcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/mangacoin.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/mangacoin.conf';
    }
  }
  return networkPath;
};
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/mangacoind.pid';
...
Bitcoin.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    async.retry({times: 1200, interval: self.startRetryInterval}, function(done) {
      if (self.node.stopping) {
        exitShutdown = true;
        return done();
      }
