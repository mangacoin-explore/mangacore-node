...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'mangacore-lib': '^' + bitcore.version,
    'mangacore-node': 'https://github.com/mangacoin-explore/mangacore-node'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/mangacore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the mangacore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the mangacore-node directory and configuration
