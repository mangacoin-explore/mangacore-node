...
var MangacoreNode = require('../node');
var index = require('../');
...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 ...
                 'There are changes necessary in both bitcoin.conf and mangacore-node.json. \n\n' +
        ...
        servicesConfig: {
          bitcoind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/mangacoind')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed mangacore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './mangacore-node.json');
  ...
  var node = new MangacoreNode(fullConfig);
...
function loadModule(req, service) {
  try {
    // first try in the built-in mangacore-node services directory
    ...
    if (servicePackage.mangacoreNode) {
      serviceModule = service.name + '/' + servicePackage.mangacoreNode;
    }
